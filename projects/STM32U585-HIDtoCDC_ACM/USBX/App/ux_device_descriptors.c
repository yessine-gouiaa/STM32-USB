...
/* USER CODE BEGIN PV0 */
static uint8_t g_usbd_mode = 0; /* default HID */
static uint8_t UserClassInstance_HID[] = {
  CLASS_TYPE_HID,
  CLASS_TYPE_NONE,   /* terminator */
};

static uint8_t UserClassInstance_CDC[] = {
  CLASS_TYPE_CDC_ACM,
  CLASS_TYPE_NONE,   /* terminator */
};
/* USER CODE END PV0 */
                                              ...
                                              uint8_t Speed)
{
  ...
  /* ===== NEW: reset class management state ===== */
  pdev->classId    = 0U;
  pdev->NumClasses = 0U;
  for (uint32_t i = 0; i < USBD_MAX_SUPPORTED_CLASS; i++)
  {
    pdev->tclasslist[i].Active = 0U;
    pdev->tclasslist[i].NumIf  = 0U;
    pdev->tclasslist[i].NumEps = 0U;
    /* Optional: clear interface/EP arrays if you want */
    for (uint32_t j = 0; j < USBD_MAX_CLASS_INTERFACES; j++)
      pdev->tclasslist[i].Ifs[j] = 0U;
    for (uint32_t j = 0; j < USBD_MAX_CLASS_ENDPOINTS; j++)
    {
      pdev->tclasslist[i].Eps[j].add     = 0U;
      pdev->tclasslist[i].Eps[j].type    = 0U;
      pdev->tclasslist[i].Eps[j].size    = 0U;
      pdev->tclasslist[i].Eps[j].is_used = 0U;
    }
  }
  /* ============================================= */
